...
- Generating HTML comparison reports for TTT hypothesis vs target DFA
- Rendering source and hypothesis DFAs side-by-side for equivalence analysis
- Displaying comparison status and counterexample information in report form
- Saving timestamped comparison artifacts for TTT evaluation runs
...
from utils import _load_html
from output_paths import get_artifact_dir


def write_ttt_comparison_html(
    source_dfa: DFA,
    hyp_dfa: DFA,
    ok: bool,
    counterexample: Optional[str],
) -> str:
    out_dir = get_artifact_dir("TTT_comparisons")
...
<body>
  <div class="top">
    <div class="kv">
      ...
      <div class="pill {"good" if ok else "bad"}">{status}</div>
      <div class="k">Counterexample:</div>
      <div><code>{cex}</code></div>
  ...
  <div class="row">
    <div class="panel">
      ...
      <div class="cnt"><iframe srcdoc="{src_html.replace('"', '&quot;')}"></iframe></div>
    </div>
    <div class="panel">
      <div class="hdr">Hypothesis DFA</div>
      <div class="cnt"><iframe srcdoc="{hyp_html.replace('"', '&quot;')}"></iframe></div>
    </div>
  </div>
</body>
</html>
