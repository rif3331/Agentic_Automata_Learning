...
def _extract_first_html_reference(value: str) -> str:
    ...
    # Excel/Sheets formula generated by previous versions:
    # =HYPERLINK("html/session_x.html","html/session_x.html")
    m = re.search(r'=HYPERLINK\(\s*"([^"]+?\.html(?:#[^"]*)?)"', text, flags=re.IGNORECASE)
    if m:
        return m.group(1)

    # Plain href/src attribute fragments.
    m = re.search(r'(?:href|src)\s*=\s*["\']([^"\']+?\.html(?:#[^"\']*)?)["\']', text, flags=re.IGNORECASE)
    if m:
        return m.group(1)

    # Any common absolute/local path or URL ending in .html.
    patterns = [
        r'file:/{2,3}[^\s,;"\'<>]+?\.html(?:#[^\s,;"\'<>]*)?',
...
def _make_clickable_csv_value(value: str, sid: str, out_dir: Path) -> str:
    ...
    if ref:
        ...
        localized = _normalize_zip_artifact_path(localized, out_dir)
...
def _results_html_table_for_zip(session_csv: Path, sid: str, out_dir: Path) -> str:
    """Create a clickable HTML table with self-contained artifact links.

    Important: when users open files directly from a ZIP on Windows, every click
    may be extracted into a different Temp folder. Therefore table links use
    embedded data URLs instead of file:/// paths or ordinary relative paths.
    """
    if not session_csv.exists():
    ...
    def embedded_href_for(localized: str) -> str:
        local = _normalize_zip_artifact_path(localized, out_dir).replace("\\", "/")
    ...
    def local_html_from_value(value: object) -> str:
        ...
        if local.endswith(".html") and (out_dir / local).exists():
            return local
        return ""

    def cell(value: object) -> str:
        raw_value = str(value or "")
        local = local_html_from_value(raw_value)
        localized_label = _localize_path_for_results_zip(raw_value, sid, out_dir)
        escaped_label = html_lib.escape(localized_label, quote=True)

        if local:
            href = embedded_href_for(local)
            label = html_lib.escape(local, quote=True)
            return (
                f'<a href="{html_lib.escape(href, quote=True)}" target="_blank" '
                f'rel="noopener" title="{escaped_label}">{label}</a>'
            )
        return escaped_label

    # Add a convenience column so users always have a clear clickable artifact,
    # even if the original CSV column name is unclear.
    extra_col = "Open HTML"
    render_fields = list(fieldnames)
    if extra_col not in render_fields:
        render_fields.append(extra_col)

    head = "".join(f"<th>{html_lib.escape(str(h), quote=True)}</th>" for h in render_fields)
    body_rows = []
    for row in rows:
        first_html = ""
        for h in fieldnames:
            first_html = local_html_from_value(row.get(h, ""))
            if first_html:
                break
        rendered_row = dict(row)
        rendered_row[extra_col] = first_html
        body_rows.append("<tr>" + "".join(f"<td>{cell(rendered_row.get(h, ''))}</td>" for h in render_fields) + "</tr>")
...
<p>Use the <b>Open HTML</b> column. Links are embedded so they keep working even when Windows opens each ZIP file from a different Temp folder.</p>
<table>
...
def _localized_html_content_for_zip(
    ...
    *,
    ...
    """Rewrite HTML artifacts so all internal links work after ZIP download.

    The important part is that internal .html links are converted to data URLs.
    This avoids broken links caused by Windows extracting each clicked ZIP member
    ...
    path = path.resolve()
    cache_key = f"{path}|{depth}|{int(embed_html_links)}|{max_depth}"
    ...
    # Also rewrite quoted JavaScript/string literals that contain artifact HTML
    # paths. Some reports store EQ/SIM links inside JS variables, not href attrs.
    def repl_quoted_string(match: re.Match[str]) -> str:
        ...
        if should_rewrite_url(value):
            ...
            # Do not HTML-escape inside JS/string literals; keep it as a normal string.
            return quote_char + new_value.replace("\\", "\\\\").replace(quote_char, "\\" + quote_char) + quote_char
        return match.group(0)

    content = re.sub(r'(["\'])([^"\']*?(?:file:/{2,3}|/html_artifact\?|/local_file\?|/opt/render/|AppData/Local/Temp|runs/sessions/|(?:html|DFA|evaluations|language_similarity_details|L_star_comparisons|TTT_comparisons)/)[^"\']*?\.html(?:#[^"\']*)?)\1', repl_quoted_string, content, flags=re.IGNORECASE)
    ...
    for pattern in plain_patterns:
        content = re.sub(pattern, repl_plain, content, flags=re.IGNORECASE)

    # Add a small warning banner in downloaded reports so it is obvious these
    # files were rewritten for offline ZIP usage.
    if "</body>" in content.lower() and "downloaded-zip-link-fix" not in content:
        banner = (
            '<div id="downloaded-zip-link-fix" style="position:fixed;right:10px;bottom:10px;'
            'z-index:999999;background:#eef6ff;border:1px solid #93c5fd;color:#1e3a8a;'
            'font:12px Arial,sans-serif;padding:6px 8px;border-radius:8px;opacity:.88">'
            'Downloaded copy: internal HTML links are embedded for offline use.</div>'
        )
        content = re.sub(r"</body>", banner + "</body>", content, count=1, flags=re.IGNORECASE)
